  ...
  return (
    <header style={{ background: '#0f0f0f', borderBottom: '1px solid #1e1e1e', position: 'sticky', top: 0, zIndex: 30 }}>
      <div style={{ maxWidth: '1200px', margin: '0 auto', padding: '10px 16px', display: 'flex', alignItems: 'center', gap: '8px', flexWrap: 'wrap' }}>
        {showBack && (
          <button onClick={onBack} style={{ background: 'none', border: 'none', color: '#cc1a1a', fontSize: '22px', padding: '2px 6px', cursor: 'pointer' }}>â†</button>
        )}
        ...
        <nav style={{ display: 'flex', gap: '4px', marginLeft: 'auto', alignItems: 'center', flexWrap: 'wrap' }}>
          {navBtn('catalog', 'CatÃ¡logo')}
          {navBtn('galeria', 'ðŸ“¸ GalerÃ­a')}
          {navBtn('info', 'Info')}
          <button onClick={() => setPage('favorites')} style={{
            ...
            â¤ï¸ Favoritas {favCount > 0 && <span style={{ background: '#cc1a1a', color: '#fff', borderRadius: '10px', padding: '1px 6px', fontSize: '11px' }}>{favCount}</span>}
          </button>
          ...
          {isAdmin && navBtn('admin', 'âš™ï¸ Admin')}
          <button onClick={onCartClick} style={{
            ...
            ðŸ›’ {cartCount > 0 && <span style={{ background: '#cc1a1a', borderRadius: '10px', padding: '1px 7px', fontSize: '11px' }}>{cartCount}</span>}
          </button>
        </nav>
      </div>
    </header>
